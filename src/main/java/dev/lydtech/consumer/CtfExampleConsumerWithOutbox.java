...
@Component
public class CtfExampleConsumerWithOutbox {

    final AtomicInteger counter = new AtomicInteger();
    final CtfExampleService ctfExampleService;

    @KafkaListener(topics = "ctf-example-inbound-with-outbox-topic", groupId = "kafkaConsumerGroup", containerFactory = "kafkaListenerContainerFactory")
    public void listen(@Header(KafkaHeaders.RECEIVED_MESSAGE_KEY) String key, @Payload final String payload) {
        ...
        try {
            ...
            ctfExampleService.processWithOutbox(key, event);
        } catch (Exception e) {
